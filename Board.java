...
public class Board implements Comparable<Board> {
	...
	/*
	 * To represent our board, we have a 2D array of booleans that tell us
	 * whether a space is occupied or not. We have a HashSet that contains
	 * blocks which we use to compare boards. The parent board is used to travel
	 * back up our "tree" to get the path once we reach the goal. The rank is
	...
	/*
	 * This is where the board is parsed from the input and constructed. We also
	 * did some error checking for board here.
	 */
	...
	// This function sets the rank of a board depending on how far a board is
	...
	// This function finds the distance between two blocks using the distance
	// equation
	public int distance(Block blockA, Block blockB) {
		int first = (int) Math.abs(Math.pow(
				blockA.getTopLeft().x - blockB.getTopLeft().x, 2));
		int next = (int) Math.abs(Math.pow(
				blockA.getTopLeft().y - blockB.getTopLeft().y, 2));
	...
	/*
	 * This method alters the boolean array and represents a block moving
	 * vertically. A negative margin means moving up. A positive margin means
	 * moving down.
	 */
	public void moveBlockVertical(Block myBlock, int margin) {
		...
		for (int i = (int) myBlock.getTopLeft().getX(); i <= myBlock
				.getBottomRight().getX(); i++) {
			for (int a = (int) myBlock.getTopLeft().getY(); a <= myBlock
					.getBottomRight().getY(); a++) {
	...
	/*
	 * This method alters the boolean array and represents a block moving
	 * horizontally. A negative margin means moving left. A positive margin
	 * means moving right.
	 */
	...
	/*
	 * This function creates a new board that is almost the same, except that
	 * the HashSet is updated by the move, which is passed in as a parameter.
	 */
		...
		for (Block b : blocks) {
			if (b.equals(m.myBlock)) {
				if (m.oneDirection.equals("up")) {
					copy.blocks.add(new Block(b.getTopLeft().x - 1, b
							.getTopLeft().y, b.getBottomRight().x - 1, b
							.getBottomRight().y));
					...
					copy.blocks.add(new Block(b.getTopLeft().x,
							b.getTopLeft().y - 1, b.getBottomRight().x, b
									.getBottomRight().y - 1));
	...
	public void removeBlock(Block myBlock) {
		...
		for (int i = (int) myBlock.getTopLeft().getX(); i <= myBlock
				.getBottomRight().getX(); i++) {
			for (int k = (int) myBlock.getTopLeft().getY(); k <= myBlock
					.getBottomRight().getY(); k++) {
				board[i][k] = false;
			}
		}
	}
	...
	public ArrayList<Move> possibleMoves() {
		...
		for (Block s : blocks) {
			...
			for (int k = (int) s.getTopLeft().getY(); k <= (int) s
					.getBottomRight().getY(); k++) {
				// up
				if (s.getTopLeft().getX() == 0)
					U++;
				try {
					if (board[(int) (s.getTopLeft().getX() - 1)][k]) {
						U++;
					}
				} catch (Exception e) {
				}
	...
	/*
	 * This hashcode uses the built-in hashcode for hashsets.
	 */
	public int hashCode() {
	...
	public void isOK() throws IllegalBoardException {
		...
		for (Block current : blocks) {
			if (!current.isOk()) {
				throw new IllegalStateException("Block: " + current + " is malformed");
			}
			...
			if (xcoordTL < 0 || ycoordTL < 0 || xcoordBR < 0 || ycoordBR < 0) {
				throw new IllegalBoardException("Block:" + current
						+ " is out of bounds");
			}
			if (xcoordTL > board.length || ycoordTL > board[0].length
					|| xcoordBR > board.length || ycoordBR > board[0].length) {
				throw new IllegalBoardException("Block:" + current
						+ " is out of bounds");
			}
			for (short i = xcoordTL; i <= xcoordBR; i++) {
			...
			for (int k = 0; k < existing[0].length; k++) {
				if (board[i][k] != existing[i][k]) {
					throw new IllegalBoardException(
							"blocks hashset does not match board boolean matrix. i: "
									+ i + " k: " + k);
				}
			}
		}
		/*
		 * PRINT EXISTING for (short i = 0; i < existing.length; i++) { for
		 * (short k = 0; k < existing[0].length; k++) {
		 * System.out.print(existing[i][k] + " "); } System.out.println(); }
		 */
	}
	...
	public static void main(String[] args) throws IllegalBoardException {
		...
		System.out.println("before move vertical "
				+ myBlock.getTopLeft().getY());
		test.moveBlockVertical(myBlock, -1);
		System.out
				.println("after move vertical " + myBlock.getTopLeft().getY());
		test.printboard();
		test.isOK();
